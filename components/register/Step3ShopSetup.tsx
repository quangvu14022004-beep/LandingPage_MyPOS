...
export default function Step3ShopSetup({
  data, setData,
  hasLodging, setHasLodging,
  hasSales, setHasSales,
  onNext, onBack, loading,
  r, CITIES, DISTRICTS, businessTypes
}: any) {
  ...
  const handleFinish = () => {
    ...
    // Ghép địa chỉ đầy đủ:
    // để geocoding service tìm tọa độ chính xác
    const fullAddress = [
      data.address,  // số nhà, tên đường
      ward,          // local state phường/xã
      data.city,     // tỉnh/thành phố
    ].filter(Boolean).join(', ');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {/* ===== ĐỊA CHỈ - 2 CẤP MỚI (bỏ quận/huyện theo chính sách 01/7/2025) ===== */}
        ...
        <div>
          ...
          <select
            style={{ width: '100%', padding: '12px 16px', border: '2px solid #DBEAFE', borderRadius: '12px', fontSize: '15px', outline: 'none', background: '#FAFAFA', cursor: 'pointer' }}
            value={data.city}
            onChange={e => {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <AuthInput
            label="Phường / Xã *"
            placeholder="VD: Thanh Khê"
            value={ward}
            disabled={!data.city}
            onChange={e => setWard(e.target.value)}
          />
          <AuthInput
            label={`${r.address} * (số nhà, tên đường)`}
            placeholder="VD: 23 Dũng Sĩ Thanh Khê"
            value={data.address ?? ''}
